...
@testset "3D SLIC reconstruction" begin
    ...
    dv = vg.grid.dx * vg.grid.dy * vg.grid.dz
    for (i, j, k) in mixed
        @test isone(abs(vg.nxint[i, j, k]) + abs(vg.nyint[i, j, k]) + abs(vg.nzint[i, j, k]))
        @test count(!iszero, (vg.nxint[i, j, k], vg.nyint[i, j, k], vg.nzint[i, j, k])) == 1
        @test isfinite(vg.cplic[i, j, k])
